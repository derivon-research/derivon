...
全局参数为 `--input`、`--pretty`、`--max-input-bytes`、`--max-value-bytes`、`-h`/`--help` 和 `-v`/`--version`。graph 默认来自 stdin；存在 `--input` 时不检查 stdin。字节限制必须为大于零的十进制数。业务参数只提供稳定长名称。v0.1.0 的 `-V` 仍作为隐藏兼容 alias 接受；新脚本使用 `-v` 或 `--version`。
